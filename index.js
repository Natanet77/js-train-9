...
function findElementGreaterThan(arr, num) {
  ...
  if (foundElement) {
    ...
    return `Число, яке задовільняе умову не знайдено`;
  }
...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = arr.findIndex(({ name }) => name === "Petro");
  if (foundIndex) {
    return foundIndex;
  } else {
    return -1;
...
function getArrayKeys(arr) {
  ...
  const result = Array.from(arrayKeys);
...
function getPositiveArrayValues(arr) {
  ...
  const getArray = arr.values();
  const result = Array.from(getArray);
  const positiveArrayValues = result.filter((item) => item > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const removeId = arr.splice(startIdx);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sumElement = lastElement.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue
  );
...
function insertElementAt(arr, index, element) {
  ...
  const elementAT = arr.splice(index, 0, element);
  return arr;
